#!/usr/bin/env bash
# Verifies the ACK Rancher server and the imported downstream clusters.
...
echo ""
echo "================================================================"
echo "  ACK Platform Track - Rancher Verification (Module 14)"
echo "================================================================"
...
[[ "${READY_REPLICAS}" == "${ACK_RANCHER_REPLICAS}" ]] \
  && check_pass "Rancher has ${READY_REPLICAS}/${ACK_RANCHER_REPLICAS} ready replica(s)" \
  || check_fail "Rancher has ${READY_REPLICAS:-0}/${ACK_RANCHER_REPLICAS} ready replica(s)"
...
kubectl get secret bootstrap-secret -n cattle-system >/dev/null 2>&1 \
  && check_pass "Rancher bootstrap secret exists" \
  || check_warn "Rancher bootstrap secret is absent; the initial admin password may already have been rotated"
...
if curl --fail --silent --show-error --max-time 20 "https://${ACK_RANCHER_HOSTNAME}/ping" | grep -qx 'pong'; then
  check_pass "Rancher is reachable through https://${ACK_RANCHER_HOSTNAME}"
else
  check_fail "Rancher is not reachable through https://${ACK_RANCHER_HOSTNAME}; verify the Cloudflare public hostname route"
fi
...
elif [[ -n "${ACK_RANCHER_EXPECTED_DOWNSTREAMS}" ]]; then
  IFS=',' read -r -a downstream_clusters <<<"${ACK_RANCHER_EXPECTED_DOWNSTREAMS}"
...
if (( FAIL > 0 )); then
  echo -e "${RED}  ACK Module 14 is not complete.${NC}"
  exit 1
fi

echo -e "${GREEN}  ACK Module 14 complete! Rancher: https://${ACK_RANCHER_HOSTNAME}${NC}"
