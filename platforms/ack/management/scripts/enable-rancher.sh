...
echo ""
echo "================================================================"
echo "  ACK Rancher Management Cluster - Install Rancher"
echo "================================================================"
echo ""
...
if ! kubectl get deployment cert-manager -n cert-manager >/dev/null 2>&1; then
  log_info "Installing cert-manager v${ACK_MANAGEMENT_CERT_MANAGER_VERSION}..."
  kubectl apply -f "https://github.com/cert-manager/cert-manager/releases/download/v${ACK_MANAGEMENT_CERT_MANAGER_VERSION}/cert-manager.crds.yaml"
  helm upgrade --install cert-manager jetstack/cert-manager \
    --version "v${ACK_MANAGEMENT_CERT_MANAGER_VERSION}" \
...
log_info "Installing Rancher ${ACK_MANAGEMENT_RANCHER_CHART_VERSION} with ${ACK_MANAGEMENT_RANCHER_REPLICAS} replica(s)..."
...
helm upgrade --install rancher rancher-stable/rancher \
  --version "${ACK_MANAGEMENT_RANCHER_CHART_VERSION}" \
  --namespace cattle-system --create-namespace \
  --set hostname="${ACK_MANAGEMENT_RANCHER_HOSTNAME}" \
  --set replicas="${ACK_MANAGEMENT_RANCHER_REPLICAS}" \
...
echo "  https://${ACK_MANAGEMENT_RANCHER_HOSTNAME} -> http://rancher.cattle-system.svc.cluster.local:80"
echo ""
