#!/usr/bin/env bash
# =============================================================================
# AKS Platform Track — enable-cf-tunnel.sh
#
# Runs Cloudflare Tunnel connector pods in-cluster, authenticated with a
# tunnel token generated in the Cloudflare Zero Trust dashboard. This is an
# alternative ingress path to enable-networking.sh's public LoadBalancer +
# cert-manager/ACME setup — useful when a domain's DNS is already proxied
# through Cloudflare (orange-clouded) rather than pointed directly at the
# Gateway's LoadBalancer IP, since ACME HTTP-01 can't complete against a
# Cloudflare-proxied record that doesn't yet resolve to this cluster.
#
...
# aks.env (gitignored) and only ever lands in a live Kubernetes Secret.
# =============================================================================
...
require_cluster
[[ -n "$CF_TUNNEL_TOKEN" ]] || die "Set CF_TUNNEL_TOKEN in platforms/aks/config/aks.env first."

echo ""
echo "================================================================"
echo "  AKS Platform Track — Cloudflare Tunnel"
echo "================================================================"
echo ""
