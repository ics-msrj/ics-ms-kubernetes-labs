...
require_config
require_cluster
[[ -n "${AKS_MANAGEMENT_CF_TUNNEL_TOKEN}" ]] \
  || die "Set AKS_MANAGEMENT_CF_TUNNEL_TOKEN in ${CONFIG_FILE}."

kubectl create namespace cloudflare-tunnel --dry-run=client -o yaml | kubectl apply -f -
kubectl create secret generic cloudflared-token --namespace cloudflare-tunnel \
  --from-literal="TUNNEL_TOKEN=${AKS_MANAGEMENT_CF_TUNNEL_TOKEN}" \
...
echo ""
echo "Next: bash platforms/aks/management/scripts/platform-track.sh enable-rancher"
