#!/usr/bin/env bash
# =============================================================================
# Rancher Management Cluster — full verification.
#
# Combines modules/01-cluster-setup/scripts/verify.sh's kubeadm/Cilium health
# checks (unmodified: 1 control-plane + >=1 worker, same as module 01) with
# verify-rancher.sh's Rancher-readiness checks.
# =============================================================================
...
echo ""
echo "================================================================"
echo "   Rancher Management Cluster — kubeadm cluster verification"
echo "================================================================"
echo ""
...
if kubectl cluster-info &>/dev/null; then
  ...
  echo ""
  echo "  Hint: bash platforms/aks/management/scripts/platform-track.sh export-kubeconfig"
  echo ""
  echo "================================================================"
  exit 1
fi
...
if (( WORKER_COUNT >= 1 )); then
  ...
  check_fail "No worker nodes found — Rancher/cert-manager/cloudflared need somewhere to schedule that isn't fighting the control-plane for resources"
fi

if (( NODES_READY == NODES_TOTAL )) && (( NODES_TOTAL > 0 )); then
  check_pass "All ${NODES_TOTAL} node(s) are Ready"
else
...
echo ""
echo "================================================================"
echo -e "   kubeadm results: ${GREEN}${PASS} passed${NC}  |  ${RED}${FAIL} failed${NC}  |  ${YELLOW}${WARN} warnings${NC}"
echo "================================================================"

if (( FAIL > 0 )); then
  echo ""
  echo -e "${RED}  kubeadm cluster is not healthy — fix the FAIL items above before running Rancher checks.${NC}"
  echo ""
  exit 1
fi

echo ""
exec "${SCRIPT_DIR}/verify-rancher.sh"
