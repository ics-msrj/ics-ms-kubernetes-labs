...
echo ""
echo "================================================================"
echo "  Rancher Management Cluster - Rancher Verification"
echo "================================================================"
...
[[ "${READY_REPLICAS}" == "${AKS_MANAGEMENT_RANCHER_REPLICAS}" ]] \
  && check_pass "Rancher has ${READY_REPLICAS}/${AKS_MANAGEMENT_RANCHER_REPLICAS} ready replica(s)" \
  || check_fail "Rancher has ${READY_REPLICAS:-0}/${AKS_MANAGEMENT_RANCHER_REPLICAS} ready replica(s)"
...
if curl --fail --silent --show-error --max-time 20 "https://${AKS_MANAGEMENT_RANCHER_HOSTNAME}/ping" | grep -qx 'pong'; then
  check_pass "Rancher is reachable through https://${AKS_MANAGEMENT_RANCHER_HOSTNAME}"
else
  check_fail "Rancher is not reachable through https://${AKS_MANAGEMENT_RANCHER_HOSTNAME}; verify the Cloudflare public hostname route"
fi
...
elif [[ -n "${AKS_MANAGEMENT_EXPECTED_DOWNSTREAMS}" ]]; then
  IFS=',' read -r -a downstream_clusters <<<"${AKS_MANAGEMENT_EXPECTED_DOWNSTREAMS}"
