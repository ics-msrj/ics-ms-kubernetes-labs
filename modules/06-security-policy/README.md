...
**Pod Security Admission answers "is this pod spec safe," at creation time only.** The three Pod Security Standards — `privileged`, `baseline`, `restricted` — are built into the API server, no controller to install. `restricted` is the strict end: no privileged containers, must run as non-root, must drop all Linux capabilities, no host namespaces, and every container must set a `seccompProfile`. Every container Online Boutique ships already sets `runAsNonRoot: true`, `allowPrivilegeEscalation: false`, `capabilities.drop: [ALL]` (visible in `workloads/online-boutique/upstream/kubernetes-manifests.yaml` since Module 02) — three of the four requirements. It does **not** set `seccompProfile` — a real gap, found by actually scaling a Deployment after this module's PSA label was applied (Module 07's own cartservice scale-to-2 step, specifically) and hitting `FailedCreate ... must set securityContext.seccompProfile.type`, not by reading the spec. `setup.sh`'s Step 1 patches `seccompProfile: {type: RuntimeDefault}` onto every Deployment/StatefulSet's pod template immediately after applying the label, precisely because the vendored manifest doesn't cover it — that's *why* labeling the namespace `restricted` in this module doesn't break anything, not (only) because upstream was already fully compliant. The label itself only affects pods **created after** it's applied — it doesn't re-evaluate what's already running, which `verify.sh` confirms by checking existing pods are untouched while a freshly-attempted privileged pod gets rejected.
