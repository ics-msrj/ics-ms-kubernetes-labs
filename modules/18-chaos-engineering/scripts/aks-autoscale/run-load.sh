...
SCENARIO="${1:?Usage: run-load.sh <gradual|spike>}"
case "${SCENARIO}" in
...
if [[ "${SCENARIO}" != "baseline" ]]; then
  kubectl -n "${NAMESPACE}" get hpa autoscale-target >/dev/null || die "Run enable-hpa after reviewing VPA recommendations first."
fi
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/part-of: aks-autoscale-simulation
